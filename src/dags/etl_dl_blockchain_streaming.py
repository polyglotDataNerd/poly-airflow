...
from tools.poly_utils import get_task_status
...
dag = DAG(
    dag_id='etl_dl_blockchain_streaming',
    ...
    tags=["data-platform", "blockchain", "spark-streaming", "ethereum", "consumer"]
)
...
SPARK_STEPS = [
    {
        'Name': 'Ethereum Spark Streaming Consumer',
        ...
        "HadoopJarStep": {
            ...
            "Args": [
                "spark-submit",
                "--class",
                "com.poly.spark.Entry",
                "s3://poly-data-utils/spark/poly-spark-1.4.jar",
                "prod-Etherum",
                "s3://poly-lakehouse/blockchain/ethereum/listener/",
                "clusterEMR",
                "stream",
                "s3://poly-lakehouse/blockchain/ethereum/listener/checkpoint/",
                "blockchain_consumer"
            ],
        },
    }
...
JOB_FLOW_OVERRIDES = {
    ...
    "Instances": {
        ...
        "Ec2SubnetId": "subnet-03f95b3f909ed48f5",
    ...
    'BootstrapActions': [
        ...
        },
        {
            ...
            'ScriptBootstrapAction': {
                'Path': 's3://poly-data-utils/cloudwatch-emr/cloudwatch_stderr.sh',
            }
        },
    ],
    'Tags': [
        ...
        {
            'Key': 'Description',
            'Value': 'Blockchain Spark Streaming Consumer'
        },
        {
            'Key': 'Team',
            'Value': 'Data'
...
create_emr_cluster = EmrCreateJobFlowOperator(
    task_id="create_streaming_emr_cluster",
...
step_adder = EmrAddStepsOperator(
    ...
    job_flow_id="{{ task_instance.xcom_pull(task_ids='create_streaming_emr_cluster', key='return_value') }}",
    aws_conn_id="aws_default",
    steps=SPARK_STEPS,
    dag=dag,
)
...
step_checker = EmrStepSensor(
    task_id="watch_streams_step",
    job_flow_id="{{ task_instance.xcom_pull('create_streaming_emr_cluster', key='return_value') }}",
    step_id="{{ task_instance.xcom_pull(task_ids='add_streaming_steps', key='return_value')["
            + str(last_step)
            + "] }}",
    aws_conn_id="aws_default",
    dag=dag,
...
retry_loop = TriggerDagRunOperator(
    task_id='retry_listener_loop',
    trigger_dag_id=dag.dag_id,
    trigger_rule=TriggerRule.ONE_FAILED,
    dag=dag)

start >> create_emr_cluster >> step_adder >> step_checker >> terminate_emr_cluster >> end >> retry_loop
